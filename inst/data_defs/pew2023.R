data.spec <- list(
  file.name = "Pew Research Center Global Attitudes Spring 2023 Dataset.sav",
  file.type = 'sav',
  question.text = c(
  ...
  pre_fixups = function(data) {
    ...
    rel.vars <- str_subset(names(data), regex("^D_RELIG_", ignore_case = T)) %>%
      discard(~.x %in% c("d_relig_israel2", "d_relig_israel_jewish", "d_relig_japan_2", "d_relig_india2"))
...
cat.defs <- list(
  Party = list(
    "Missing" = c("Do not feel close to any party", "Don\u2019t know", "Refused", "Don't know", "Dont know", "DK/Refused",
                  ...
                  "No sabe (Don\u2019t know)", "No contest\u00f3 (Refused)", "Yes", "No"),
    "Other" = c("Other", "Independent", "Other (SPECIFY)", "Other party (please specify)", "Otro (Other)", "Outro (Other)",
                "Lainnya (Other)")
  ),
  Religion = list(
    "Missing" = c("Refused", "Don\u2019t know", "Don't know", "Dont know"),
    "Other" = c("Something else, or", "Confucianism, Shintoism, animism or other traditional or folk religion",
