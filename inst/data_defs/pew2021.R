data.spec <- list(
  file.name = "Pew Research Center Global Attitudes Spring 2021 Dataset - Public.sav",
  file.type = 'sav',
  question.text = c(
  ...
  fixups = function(data) {
    data <- data %>% mutate(Year = format.Date(QDATE_S, format = "%Y"))
...
cat.defs <- list(
  Party = list(
    "Missing" = c("Do not feel close to any party", "Don\u2019t know", "Refused"),
    "Other" = c("Other", "Independent", "Other party", "Other (SPECIFY)")
  ),
  Language = list(
    "Missing" = c("Refused")
  ),
  Religion = list(
    "Missing" = c("Refused", "Don\u2019t know", "Don\u0092t know", "Don't know", "Dont know"),
